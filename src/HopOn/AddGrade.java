...
/*            AuthorizeUser authent = new AuthorizeUser(conn);
            ...
            } else if (Action.equals("GetGradeInfo")) {
                this.GetGradeInfo(request, out, conn, context, UserId);
            } else {
    ...
    private void GetInformation(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex) {
        ...
        try {
            ...
            Query = "SELECT Id,Grade,(CASE WHEN Status=0 THEN 'Active' ELSE 'Inactive' END) " +
                    " FROM Grades WHERE Status = 0 ORDER BY Id ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
                GradeList.append("<tr>");
                GradeList.append("<td>" + SrlNo + "</td>");
                GradeList.append("<td>" + rset.getString(2) + "</td>");
                GradeList.append("<td>" + rset.getString(3) + "</td>");
                GradeList.append("<td><i class=\"fa fa-edit\" data-toggle=\"modal\" data-target=\"#myModal\" onClick=\"editRow(" + rset.getInt(1) + ")\"></i></td>");
                GradeList.append("</tr>");
                ++SrlNo;
            }
            rset.close();
            ...
            var20.SetField("GradeList", GradeList.toString());
            var20.GenerateHtml(out, Supportive.GetHtmlPath(servletContext) + "Forms/AddGrade.html");
            out.flush();
            out.close();
        } catch (Exception var19) {
            Supportive.DumException("AddGrade", "First Method -- GetInfo--MAIN", request, var19, getServletContext());
            try {
                Parsehtm Parser = new Parsehtm(request);
                Parser.SetField("FormName", "AddGrade");
        ...
        if (count == 0) {
            try {
                pStmt = conn.prepareStatement(
                        "INSERT INTO Grades (Grade, Status, CreatedDate, CreatedBy) " +
                                "VALUES (?,?,now(),?)");
                ...
                Supportive.DumException("AddGrade", "Second Method -- Saving Information -- 001", request, e, getServletContext());
                try {
                    Parsehtm Parser = new Parsehtm(request);
                    Parser.SetField("FormName", "AddGrade");
                    Parser.SetField("ActionID", "GetInfo");
            ...
            Query = request.getParameter("GradeIndex").trim();
            int GradeIndex = Integer.parseInt(Query);
            try {
                Query = "UPDATE Grades SET Grade='" + Grade + "',Status=" + Status + ", " +
                        " WHERE Id = " + GradeIndex;
                stmt = conn.createStatement();
                stmt.executeUpdate(Query);
                stmt.close();
            } catch (Exception e) {
                Supportive.DumException("AddGrade", "Second Method -- Saving Information -- 002", request, e, getServletContext());
                try {
                    Parsehtm Parser = new Parsehtm(request);
                    Parser.SetField("FormName", "AddGrade");
                    Parser.SetField("ActionID", "GetInfo");
    ...
    void GetGradeInfo(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId) {
        Query = "";
        stmt = null;
        rset = null;
        String chk = "";
        ...
        try {
            Query = "Select Id,Grade,Status from Grades where Id=" + GradeIndex;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
                Grade = rset.getString(2);
                Status = rset.getString(3);
            }
            rset.close();
            stmt.close();

            out.println(Grade + "|" + GradeIndex + "|" + Status);
            out.flush();
            out.close();

        } catch (Exception var19) {
            Supportive.DumException("AddGrade", "Third Method -- AddGrade--001", request, var19, getServletContext());
            try {
                Parsehtm Parser = new Parsehtm(request);
                Parser.SetField("FormName", "AddGrade");
                Parser.SetField("ActionID", "GetInfo");
