...
public class AssignRights extends HttpServlet {
    ...
    private void GetUserInfo(HttpServletRequest request, PrintWriter out, Connection conn, String UserId, int cityIndex, int isAdmin) throws FileNotFoundException {
        Statement stmt = null;
        ResultSet rset = null;
        int ListCount = 0;
        String Query = "";
        ...
        if (isAdmin == 1) {
            ...
            Query = "SELECT Id, UserName FROM SystemUsers WHERE Status=0 AND CityIndex = " + cityIndex + " ORDER BY UserName ";
        }
        ...
        if (isAdmin == 1) {
            ...
            Query = "SELECT max(Indexptr) FROM ScreenNames WHERE SystemIndex=2 AND CityIndex = " + cityIndex + " ";
        }
        ...
        if (isAdmin == 1) {
            Query = "SELECT Indexptr, ScreenName, ScreenLevel FROM ScreenNames WHERE SystemIndex=2  ORDER BY DisplayOrder, ScreenName ";
        } else {
            Query = "SELECT Indexptr, ScreenName, ScreenLevel FROM ScreenNames WHERE SystemIndex=2 AND CityIndex = " + cityIndex + " " + " ORDER BY DisplayOrder, ScreenName ";
        }
    ...
    private void SaveUserRights(HttpServletRequest request, PrintWriter out, Connection conn, String UserId, int cityIndex) throws IOException {
        Statement stmt = null;
        ResultSet rset = null;
        int i = 0, opt = 0;
        String Query = "", chk = "", RightsString = "";
        ...
        for (i = 0; i <= ScreenCount; i++) {
            ...
            try {
                PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO UserRights (ClientIndex, SystemIndex, EmployeeIndex, ScreenNo, UserRights,CityIndex) \n" +
                                "VALUES (1,2,?,?,?,?) ");
                MainReceipt.setInt(1, Indexptr);
                MainReceipt.setInt(2, i);
                MainReceipt.setInt(3, opt);
                MainReceipt.setInt(4, cityIndex);
