...
/*            AuthorizeUser authent = new AuthorizeUser(conn);
            ...
            } else if (Action.equals("GetEquipmentsInfo")) {
                this.GetEquipmentsInfo(request, out, conn, context, UserId);
            } else {
    ...
    private void GetInformation(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex) {
        ...
        try {
            ...
            Query = "SELECT Id,Equipment,(CASE WHEN Status=0 THEN 'Active' ELSE 'Inactive' END) " +
                    " FROM Equipments WHERE Status = 0 ORDER BY Id ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
                EquipmentList.append("<tr>");
                EquipmentList.append("<td>" + SrlNo + "</td>");
                EquipmentList.append("<td>" + rset.getString(2) + "</td>");
                EquipmentList.append("<td>" + rset.getString(3) + "</td>");
                EquipmentList.append("<td><i class=\"fa fa-edit\" data-toggle=\"modal\" data-target=\"#myModal\" onClick=\"editRow(" + rset.getInt(1) + ")\"></i></td>");
                EquipmentList.append("</tr>");
                ++SrlNo;
            }
            rset.close();
            ...
            var20.SetField("EquipmentList", EquipmentList.toString());
            var20.GenerateHtml(out, Supportive.GetHtmlPath(servletContext) + "Forms/AddEquipments.html");
            out.flush();
            out.close();
        } catch (Exception var19) {
            Supportive.DumException("AddEquipments", "First Method -- GetInfo--MAIN", request, var19, getServletContext());
            try {
                Parsehtm Parser = new Parsehtm(request);
                Parser.SetField("FormName", "AddEquipments");
    ...
    private void SaveRecords(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId) {
        ...
        String Equipment = request.getParameter("Equipment").trim();
        ...
        if (count == 0) {
            try {
                pStmt = conn.prepareStatement(
                        "INSERT INTO Equipments (Equipment, Status, CreatedDate, CreatedBy) " +
                                "VALUES (?,?,now(),?)");
                ...
                Supportive.DumException("AddEquipments", "Second Method -- Saving Information -- 001", request, e, getServletContext());
                try {
                    Parsehtm Parser = new Parsehtm(request);
                    Parser.SetField("FormName", "AddEquipments");
                    Parser.SetField("ActionID", "GetInfo");
            ...
            Query = request.getParameter("EquipmentIndex").trim();
            int EquipmentIndex = Integer.parseInt(Query);
            try {
                Query = "UPDATE Equipments SET Equipment='" + Equipment + "',Status=" + Status + ", " +
                        " WHERE Id = " + EquipmentIndex;
                stmt = conn.createStatement();
                stmt.executeUpdate(Query);
                stmt.close();
            } catch (Exception e) {
                Supportive.DumException("AddEquipments", "Second Method -- Saving Information -- 002", request, e, getServletContext());
                try {
                    Parsehtm Parser = new Parsehtm(request);
                    Parser.SetField("FormName", "AddEquipments");
                    Parser.SetField("ActionID", "GetInfo");
    ...
    private void GetEquipmentsInfo(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId) {
        Query = "";
        stmt = null;
        rset = null;
        String chk = "";
        ...
        try {
            Query = "Select Id,Equipment,Status from Equipments where Id=" + EquipmentIndex;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
                Equipment = rset.getString(2);
                Status = rset.getString(3);
            }
            rset.close();
            stmt.close();

            out.println(Equipment + "|" + EquipmentIndex + "|" + Status);
            out.flush();
            out.close();

        } catch (Exception var19) {
            Supportive.DumException("AddEquipments", "Third Method -- AddInstructions--001", request, var19, getServletContext());
            try {
                Parsehtm Parser = new Parsehtm(request);
                Parser.SetField("FormName", "AddEquipments");
                Parser.SetField("ActionID", "GetInfo");
