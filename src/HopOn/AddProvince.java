...
/*            AuthorizeUser authent = new AuthorizeUser(conn);
            ...
            if (Action.equals("GETINPUT")) {
                ...
                this.SaveRecords(request, out, conn, context, UserId, CityIndex);
    ...
    void GetInput(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int cityIndex, int isAdmin) {
        ...
        try {
            if (isAdmin == 1) {
                this.Query = "SELECT Id,countryName FROM Country  WHERE Status=0  ORDER BY Id";
            } else {
                this.Query = "SELECT Id,countryName FROM Country  WHERE Status=0 AND CityIndex = " + cityIndex + "  ORDER BY Id";
            }
            ...
            while (this.rset.next()) {
                CountryList.append("<option value=" + this.rset.getString(1) + ">" + this.rset.getString(2) + "</option>");
            }

            this.rset.close();
            this.stmt.close();
            if (isAdmin == 1) {
                this.Query = "SELECT a.Id,a.ProvinceName,(CASE WHEN a.Status=0 THEN 'Active' ELSE 'Inactive' END), \n  a.CountryIndex,a.CreatedDate,b.countryName FROM  Province a, Country b \n WHERE a.CountryIndex=b.Id AND a.Status=0 ORDER BY a.Id";
            } else {
                this.Query = "SELECT a.Id,a.ProvinceName,(CASE WHEN a.Status=0 THEN 'Active' ELSE 'Inactive' END), \n  a.CountryIndex,a.CreatedDate,b.countryName FROM  Province a, Country b \n WHERE a.CountryIndex=b.Id AND a.Status=0 AND a.CityIndex = " + cityIndex + " ORDER BY a.Id";
            }
    ...
    void SaveRecords(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int cityIndex) {
        ...
        if (count == 0) {
            try {
                this.pStmt = conn.prepareStatement("INSERT INTO Province (ProvinceName,Status,CreatedDate,CountryIndex,CityIndex) \n  VALUES (?,?,now(),?,?)");
                this.pStmt.setString(1, ProvinceName);
                this.pStmt.setString(2, Status);
                this.pStmt.setString(3, CountryIndex);
                this.pStmt.setInt(4, cityIndex);
