...
public class DashboardTechnician extends HttpServlet {
    ...
    private void Services(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (ActionID.equals("TechnicianDashboard")) {
                TechnicianDashboard(request, out, conn, context, UserId, CityIndex, isAdmin);
            } else if (ActionID.equals("ShowLoggedInTech")) {
                ShowLoggedInTech(request, out, conn, context, UserId, CityIndex, isAdmin);
            } else if (ActionID.equals("ShowLoggedOutTech")) {
                ShowLoggedOutTech(request, out, conn, context, UserId, CityIndex, isAdmin);
            } else if (ActionID.equals("ShowOfflineTech")) {
                ShowOfflineTech(request, out, conn, context, UserId, CityIndex, isAdmin);
            } else if (ActionID.equals("ShowUnAssignedJobs")) {
                ShowUnAssignedJobs(request, out, conn, context, UserId, CityIndex, isAdmin);
            } else if (ActionID.equals("ShowAssignedJobs")) {
                ShowAssignedJobs(request, out, conn, context, UserId, CityIndex, isAdmin);
            } else {
    ...
    private void TechnicianDashboard(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex, int isAdmin) {
        stmt = null;
        rset = null;
        Query = "";
        String Query2 = "";
        ...
        try {
            ...
            if (isAdmin == 1)
                ...
                Query = "SELECT a.Id, a.UserName, ifnull(b.Cnt,0),a.UserId FROM MobileUsers a " +
                        "LEFT JOIN (SELECT UserId, Count(*) Cnt FROM Assignment " +
                        "WHERE CreatedDate >= DATE_SUB(NOW(), INTERVAL 30 DAY) GROUP BY UserId) b ON a.Id = b.UserId  " +
                        "WHERE a.Status=0 AND a.CityIndex = " + ParamCheck + " ORDER BY a.UserName";
            try {
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                while (rset.next()) {
                    TechnicianWise.append("<tr>");
                    TechnicianWise.append("<td align='left' data-container='body' data-toggle='tooltip' data-placement='left' title= 'Click to Find Driver Location' ><a href = \"/HopOn/HopOn.ManualAssignment?ActionID=GetTechLocation&TechId=" + rset.getString(4) + " \" target=\"_self\"> <small>" + rset.getString(2) + "</small> </a></td>");
...
//                TotalTechStatus.append("<td align='left' data-container='body' data-toggle='tooltip' data-placement='left' title= 'Click to Find Technician Location' ><a href = \"/HopOn/HopOn.ManualAssignment?ActionID=GetTechLocation&TechId=" + TechId + " \" target=\"_self\"> <small>" + TechName + "</small> </a></td>");
//
...
//                if (LoginCount == 0)
//                    TotalTechStatus.append("<td class=\"text-center \"><img src=/HopOn/images/OfflineImage.png width=20 height=10 ><br></td>");// Status
//                else
//                    TotalTechStatus.append("<td class=\"text-center \" ><img src=/HopOn/images/OnlineImage.png width=20 height=10 ><br></td>");// Status
//                TotalTechStatus.append("</tr>");
//            }
//            rset1.close();
//            cStmt.close();
...
//            Parser.SetField("AssignedJobs", String.valueOf(AssignedJobs));
            ...
            Parser.SetField("CityIndex", String.valueOf(CityIndex));
            Parser.SetField("AdminCheck", String.valueOf(isAdmin));
    ...
    private void ShowLoggedInTech(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex, int isAdmin) {
        stmt = null;
        rset = null;
        Query = "";
        String Query1 = "";
        ...
        try {
            if (ParamCheck == -1)
                ...
                Query = "SELECT Id,UserName,UserId FROM MobileUsers WHERE Status=0 AND CityIndex = " + ParamCheck + " ORDER BY Id";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
                ...
                if (LoginCount == 1) {
                    ...
                    LoggedIn.append("<td><img src=/" + (ServerIndex == 1 ? "HopOn" : "TAMAPPTesting") + "/images/OnlineImage.png width=30 height=20 ><br></td>");// Status
    ...
    private void ShowLoggedOutTech(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex, int isAdmin) {
        stmt = null;
        rset = null;
        Query = "";
        String Query1 = "";
        ...
        try {
            ...
            while (rset.next()) {
                ...
                if (LoginCount == 0) {
                    ...
                    LoggedOut.append("<td><img src=/" + (ServerIndex == 1 ? "HopOn" : "TAMAPPTesting") + "/images/OfflineImage.png width=30 height=20 ><br></td>");// Status
    ...
    private void ShowOfflineTech(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex, int isAdmin) {
        stmt = null;
        rset = null;
        Query = "";
        String Query1 = "";
...
/*            Query = "SELECT b.UserId, b.UserName, a.LoginFlag, DATE_FORMAT(max(a.CreatedDate),'%Y-%m-%d %H:%i:%s'), \n" +
            ...
            if (ParamCheck == -1)
                ...
                Query = "SELECT b.UserId, b.UserName, a.LoginFlag, DATE_FORMAT(max(a.CreatedDate),'%Y-%m-%d %H:%i:%s'), \n" +
                        ...
                        "concat(timestampdiff(DAY,DATE_FORMAT(max(a.CreatedDate),'%Y-%m-%d %H:%i:%s'),DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s')),'d ',lpad (MOD(timestampdiff(HOUR,DATE_FORMAT(max(a.CreatedDate),'%Y-%m-%d %H:%i:%s'),DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s')), 24),2,'0' ),'h',lpad (MOD(timestampdiff(MINUTE,DATE_FORMAT(max(a.CreatedDate),'%Y-%m-%d %H:%i:%s'),DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s')), 60),2,'0' ),'m' ) AS TotalTime \n" +
                        "FROM TechnicianLoginTrail a, MobileUsers b, (SELECT TechId, MAX(Id) Id FROM TechnicianLoginTrail GROUP BY TechId) x\n" +
                        "WHERE a.TechId = b.UserId AND b.`Status` = 0 AND a.LoginFlag = 'Offline' AND a.TechId=x.TechId AND a.Id=x.Id AND a.CityIndex = " + ParamCheck + " \n" +
                        "GROUP BY  b.UserId, b.UserName, a.LoginFlag " +
                        "ORDER BY TIMEDIFF(DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s'),DATE_FORMAT(max(a.CreatedDate),'%Y-%m-%d %H:%i:%s')) DESC ";
...
//                if (rset1.next()) {
                ...
                Offline.append("<td align=center><img src=/" + (ServerIndex == 1 ? "HopOn" : "TAMAPPTesting") + "/images/Offline.png width=30 height=40 ><br></td>");// Status
    ...
    private void ShowUnAssignedJobs(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex, int isAdmin) {
        stmt = null;
        rset = null;
        Query = "";
        String ReplaceCN = "";
        ...
        try {
            ...
            if (ParamCheck == -1)
                ...
                Query = "SELECT a.RegistrationNum,a.ChesisNo,a.CustName,IFNULL(a.AssignDate,'0000-00-00'),a.Make," +
                        ...
                        " FROM CustomerData a " +
                        " STRAIGHT_JOIN JobType e ON a.JobTypeIndex = e.Id " +
                        " WHERE a.AssignStatus = 0 AND a.CityIndex = " + ParamCheck + " " +
                        " ORDER BY a.CreatedDate DESC";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
                ...
                DuAJ.append("<td align=center><a class=\"btn-sm btn btn-info\" href=/" + (ServerIndex == 1 ? "HopOn" : "TAMAPPTesting") + "/HopOn.ComplainLocation?Action=ViewLocationReport&Flag=1&ComplainNumber=" + ReplaceCN + " target=NewFrame1><i class=\"fa fa-edit\"></i> [ View ] </a></td>");
    ...
    private void ShowAssignedJobs(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex, int isAdmin) {
        stmt = null;
        rset = null;
        Query = "";
        String ReplaceCN = "";
        ...
        try {
            ...
            while (rset.next()) {
                ...
                DAJ.append("<td align='left' data-container='body' data-toggle='tooltip' data-placement='left' title= 'Click to Find Technician Location' ><a href = \"/" + (ServerIndex == 1 ? "HopOn" : "TAMAPPTesting") + "/HopOn.ManualAssignment?ActionID=GetTechLocation&TechId=" + rset.getString(13) + " \" target=\"_self\"> <small>" + rset.getString(12) + "</small> </a></td>");
                DAJ.append("<td align=center><a class=\"btn-sm btn btn-info\" href=/" + (ServerIndex == 1 ? "HopOn" : "TAMAPPTesting") + "HopOn/HopOn.ComplainLocation?Action=ViewLocationReport&Flag=1&ComplainNumber=" + ReplaceCN + " target=NewFrame1><i class=\"fa fa-edit\"></i> [ View ] </a></td>");
