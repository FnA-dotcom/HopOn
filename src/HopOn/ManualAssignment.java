...
//                    " AND a.id=b.UserId AND b.cnt < (SELECT totalcount FROM TotalJobsHand) ORDER BY a.Id ";
            ...
            UserNames.append("<option value=0 Selected Disabled> Select Driver </option>");
            while (rset.next()) {
...
/*            Query = "SELECT b.UserName,c.ComplainNumber,a.RegisterBy," +
            ...
            while (rset.next()) {
                ...
                if (rset.getInt(5) == 7 || rset.getInt(5) == 7) {
                    ...
                    ExistingData.append("<td align='left' data-container='body' data-toggle='tooltip' data-placement='left' title= 'Click to Find Technician Location' ><a href = \"/HopOn/HopOn.ManualAssignment?ActionID=GetTechLocation&TechId=" + rset.getString(6) + " \" target=\"_self\"> " + rset.getString(1) + " </a></td>");
                    ...
                    ExistingData.append("<td align=center><a class=\"btn-xs btn btn-info\" href=/HopOn/HopOn.ComplainLocation?Action=ViewLocationReport&Flag=1&ComplainNumber=" + ReplaceComplainNumber + " target=NewFrame1><i class=\"fa fa-edit\"></i> [View] </a></td>");
                    ExistingData.append("</tr>");
                } else {
                    ExistingData.append("<tr>");
                    ExistingData.append("<td>" + SrlNo + "</td>");
    ...
    private void SaveRecords(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int CityIndex) {
        stmt = null;
        rset = null;
        Query = "";
            ...
            try {
                pStmt = conn.prepareStatement(
                        "INSERT INTO AssignmentHistory(UserId, ComplaintId, Status, CreatedDate, RegisterBy,ComplaintStatus, " +
                                "AssignmentStatus,Latitude,Longtitude,ManualStatus,Distance,FalconRadius,TransferFlag,ManualDate,CityIndex) " +
                ...
                pStmt.setInt(8, CityIndex);
                pStmt.executeUpdate();
                pStmt.close();

                pStmt = conn.prepareStatement(
                        "INSERT INTO Assignment(UserId, ComplaintId, Status, CreatedDate, RegisterBy,ComplaintStatus, " +
                                "AssignmentStatus,ManualStatus,ManualDate,CityIndex) VALUES (?,?,0,now(),?,0,0,1,NOW(),?)");
