...
//                conn = DriverManager.getConnection(connect_string);
    ...
    private void GetValue(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int cityIndex, int isAdmin) {
        ...
        try {
            if (isAdmin == 1) {
                this.Query = "SELECT count(*) FROM Assignment WHERE  date_format(CreatedDate,'%Y-%m-%d') =  DATE_FORMAT(NOW(),'%Y-%m-%d') ";
            } else {
                this.Query = "SELECT count(*) FROM Assignment WHERE  date_format(CreatedDate,'%Y-%m-%d') =  DATE_FORMAT(NOW(),'%Y-%m-%d') " +
                        " AND CityIndex = " + cityIndex + " ";
            }

            this.stmt = conn.createStatement();
            this.rset = this.stmt.executeQuery(this.Query);
            if (this.rset.next()) {
                totalToday = this.rset.getInt(1);
            }
            rset.close();
            stmt.close();
            ...
            if (isAdmin == 1) {
                ...
                Query = "SELECT count(*) FROM Assignment WHERE ComplaintStatus = 6 AND " +
                        "date_format(CreatedDate,'%Y-%m-%d') =  DATE_FORMAT(NOW(),'%Y-%m-%d') AND CityIndex = " + cityIndex + " ";
            }
            ...
            if (isAdmin == 1) {
                ...
                Query = "SELECT count(*) FROM Assignment WHERE ComplaintStatus NOT IN (6,7,8) AND " +
                        "CreatedDate >=  DATE_SUB(NOW(), INTERVAL 365 DAY) AND CityIndex = " + cityIndex + "  ";
            }
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            if (isAdmin == 1) {
                ...
                Query = "SELECT count(*) FROM Assignment WHERE ComplaintStatus NOT IN (6,7,8) AND " +
                        "CreatedDate >=  DATE_SUB(NOW(), INTERVAL 30 DAY) AND CityIndex = " + cityIndex + "  ";
            }
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            if (isAdmin == 1) {
                ...
                Query = "SELECT a.Id, a.ComplaintStatus, ifnull(b.Cnt,0) FROM ComplaintStatus a " +
                        "LEFT JOIN (SELECT ComplaintStatus, Count(*) Cnt FROM Assignment " +
                        "WHERE CreatedDate >= DATE_SUB(NOW(), INTERVAL 30 DAY) GROUP BY ComplaintStatus) b ON a.Id = b.ComplaintStatus " +
                        " WHERE  a.Status=0 ORDER BY a.ComplaintStatus AND CityIndex = " + cityIndex + " ";
            }
            ...
            out.println("Error In DashBoard Screen " + Ex.getMessage());
            out.flush();
            out.close();
            return;
        }
    ...
    private void ShowAssignedJobs(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId) {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                AssignedJob.append("<td align=center><a class=\"btn-sm btn btn-info\" href=/HopOn/HopOn.ComplainLocation?Action=ViewLocationReport&Flag=1&ComplainNumber=" + ReplaceComplainNumber + " target=NewFrame1><i class=\"fa fa-edit\"></i> [ View ] </a></td>");
                AssignedJob.append("</tr>");
            }
            rset.close();
            stmt.close();
    ...
    private void ShowCompleteJobs(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId) {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                CompleteStatus.append("<td align=center><a class=\"btn-sm btn btn-info\" href=/HopOn/HopOn.ComplainLocation?Action=ViewLocationReport&Flag=1&ComplainNumber=" + ReplaceComplainNumber + " target=NewFrame1><i class=\"fa fa-edit\"></i> [ View ] </a></td>");
                CompleteStatus.append("</tr>");
            }
            rset.close();
            stmt.close();
            ...
            Parser.GenerateHtml(out, Supportive.GetHtmlPath(servletContext) + "Reports/ShowCompleteJobsDashBoard.html");
        } catch (Exception var15) {
            out.println("Exception in main... " + var15.getMessage());
            out.flush();
            out.close();
    ...
    private void ShowCanceledJobs(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId) {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                CanceledStatus.append("<td align=center><a class=\"btn-sm btn btn-info\" href=/HopOn/HopOn.ComplainLocation?Action=ViewLocationReport&Flag=1&ComplainNumber=" + ReplaceComplainNumber + " target=NewFrame1><i class=\"fa fa-edit\"></i> [ View ] </a></td>");
                CanceledStatus.append("</tr>");
            }
            rset.close();
            stmt.close();
    ...
    private void ShowOtherJobs(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId) {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                OtherStatus.append("<td align=center><a class=\"btn-sm btn btn-info\" href=/HopOn/HopOn.ComplainLocation?Action=ViewLocationReport&Flag=1&ComplainNumber=" + ReplaceComplainNumber + " target=NewFrame1><i class=\"fa fa-edit\"></i> [ View ] </a></td>");
                OtherStatus.append("</tr>");
            }
            rset.close();
            stmt.close();
