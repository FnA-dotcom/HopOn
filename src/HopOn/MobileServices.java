...
public class MobileServices extends HttpServlet {
    ...
    public void ServicesRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        } else if (ActionID.equals("UpdateFireBaseToken")) {
            UpdateFireBaseToken(request, conn, SOS, out);
        }
        try {
            assert conn != null;
            conn.close();
...
/*                    Query1 = "{CALL UpdateJobStatus(?,?)}";
        ...
        try {
            if (!Latitude.equals("0.0") || !Longitude.equals("0.0")) {
                ...
                pStmt = conn.prepareStatement(
                        "INSERT INTO TechnicianLocation (Longtitude, Latitude, UserId, Status, CreatedDate,CityIndex) " +
                                "VALUES (?,?,?,0,NOW(),?)");
        ...
        try {
            ...
            pStmt = conn.prepareStatement(
                    "INSERT INTO FaultInspection (Horn, InteriorLights, SideMirrors, PowerWindows, CentralLockingSystem, AC, " +
                            ...
                            "ToolKit, Carjack, Pic1, Pic2, Pic3, Pic4, UserId, Status, CreatedDate,ComplainNumber,CityIndex) " +
                            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,NOW(),?,?) ");
    ...
    private void QAForm(HttpServletRequest request, PrintWriter out, Connection conn) {
        ...
        try {
            ...
            pStmt = conn.prepareStatement(
                    "INSERT INTO QAForm (Ans, Ans1, Ans2, Ans3, Ans4, Ans5, Pic1, Pic2, Pic3, Pic4, ComplainNumber, " +
                            "UserId,Status,CreatedDate,CityIndex) " +
                            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,NOW(),?) ");
    ...
    private void SavePicture(HttpServletRequest request, PrintWriter out) {
        try {
            FileOutputStream fout = new FileOutputStream(new File("/opt/TAMAPPTesting.txt"), true);
            ...
            File tmpDir = null;
            //Live Server
            //tmpDir = new File(getPicsFolder(1));
            //Test Server
            //tmpDir = new File(getPicsFolder(2));
            ...
            try {
                ...
                for (Object item1 : items) {
                    ...
                    if (!item.isFormField()) {
                        //Live Server
                        //File file = new File(getPicsFolder(1), item.getName());
                        //Test Server
    ...
    private String getPicsFolder() {
        //return ServerIndex == 1 ? "/opt/Htmls/HopOn/pics/" : "/opt/Htmls/TAMAPPTesting/pics/";
        //For Live Server
        return "/opt/Htmls/HopOn/pics/";
        //For Testing Server
        //return "/opt/Htmls/TAMAPPTesting/pics/";
    }
    ...
    private void PostponedReasonInsertion(HttpServletRequest request, PrintWriter out, Connection conn) {
        ...
        try {
            ...
            int CityIndex = CityIndex(conn, UserId);
            pStmt = conn.prepareStatement(
                    "INSERT INTO PostponedReasons(PostponedReason, JobId, UserId, Status, CreatedDate, AssignmentIndex,CityIndex) " +
                            "VALUES (?,?,?,0,NOW(),?,?)");
    ...
    private void UpdateFireBaseToken(HttpServletRequest request, Connection conn, ServletOutputStream SOS, PrintWriter out) throws IOException {
        stmt = null;
        rset = null;
        Query = " ";
        pStmt = null;
        ...
        int CityIndex = 0;
        try {
            Query = "SELECT Id,CityIndex FROM MobileUsers WHERE upper(trim(UserId))='" + UserId.toUpperCase().trim() + "' ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
                TechId = rset.getInt(1);
                CityIndex = rset.getInt(2);
            }
            rset.close();
            stmt.close();
            ...
            if (UserCount == 0) {
                pStmt = conn.prepareStatement(
                        "INSERT INTO UserBindageFB(UserId, FBTokenId, Status, CreatedDate,CityIndex) " +
                                "VALUES (?,?,0,NOW(),?)");
    ...
    private int CityIndex(Connection conn, String UserId) {
        ...
        int CityIndex = 0;
        try {
            Query = "SELECT CityIndex FROM MobileUsers WHERE UserId = '" + UserId + "' ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next())
